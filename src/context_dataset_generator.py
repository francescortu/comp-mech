...
def process_sample(sample: Dict) -> Dict:
    ...
    new_sample["prompt"] = add_random_context(sample)
    # compute subject position
    return new_sample

def add_random_context(sample: Dict) -> str:
    context = generate_random_context(context_target=sample["target_new"])
    formatter = " "
    new_template = f"{formatter}{context} {sample['base_prompt']}"
    return new_template

def generate_random_context(context_target: str) -> str:
...
class ContextDatasetGPT:
    ...
    def process_dataset(self):
        ...
        for i, _ in enumerate(new_dataset):
            if i % 1000 == 0:
                self.save_dataset(new_dataset[:i+1], checkpoint=True)
