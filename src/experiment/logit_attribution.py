...
class AttributeStorage:
    ...
    def append(self, mem_attribute:torch.Tensor, cp_attribute:torch.Tensor, diff_attribute:torch.Tensor, labels, object_position:int):
        mem_attribute, cp_attribute, diff_attribute = self.aggregate(mem_attribute, cp_attribute, diff_attribute, object_position)
    ...
    def aggregate(self, mem_attribute, cp_attribute, diff_attribute, object_position:int):
        length = mem_attribute.shape[-1]
        aggregated_mem = aggregate_result(mem_attribute, object_positions=object_position, length=length)
        aggregated_cp = aggregate_result(cp_attribute, object_positions=object_position, length=length)
        aggregated_diff = aggregate_result(diff_attribute, object_positions=object_position, length=length)
...
class LogitAttribution(BaseExperiment):
    ...
    def slice_target(self, target:torch.Tensor, length:int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        target_mem = einops.repeat(target_mem, "seq_len ->  seq_len length", length=length)
        target_cp = einops.repeat(target_cp, "seq_len ->  seq_len length", length=length)
        return target_mem, target_cp
    
    def attribute_single_len(self, length:int, storage:AttributeStorage, component:str, apply_ln:bool=False):
        ...
        for batch in tqdm(dataloader, total=num_batches):
            ...
            stack_of_resid, resid_labels = cache.decompose_resid(apply_ln=apply_ln, return_labels=True, mode="attn")
            stack_of_component, labels = cache.get_full_resid_decomposition(expand_neurons=False, apply_ln=apply_ln, return_labels=True) # return a tensor of shape (component_size, batch_size, seq_len, hidden_size)
            ...
            diff_attribute = cache.logit_attrs(stack_of_component, tokens=target_mem, incorrect_tokens=target_cp) # (component, batch, position)
            object_position = self.dataset.obj_pos[0]
            storage.append(mem_attribute.cpu(), cp_attribute.cpu(), diff_attribute.cpu(), labels, object_position)
        
        # clear the cuda cache
        torch.cuda.empty_cache()
            
    def attribute(self, apply_ln:bool=False):
        ...
        for length in lengths:
            if length == 11:
                continue
            self.attribute_single_len(length, storage, "logit", apply_ln=apply_ln)
        
        return storage.stack(), storage.labels
    
    def run(self, apply_ln:bool=False):
        (mem, cp, diff), labels = self.attribute(apply_ln=apply_ln)
