...
class Ablate(BaseExperiment):
    ...
    def _get_freezed_attn(self, cache) -> Dict[str, Tuple[torch.Tensor, Any]]:
        ...
        for layer in range(self.model.cfg.n_layers):
            hooks[f"L{layer}"] = (
                ...
                partial(freeze_hook, clean_attn_activation = cache[f"blocks.{layer}.hook_attn_out"])
            )
    ...
    def _get_freezed_attn_pattern(self, cache) -> Dict[str, Tuple[torch.Tensor, Any]]:
        ...
        for layer in range(self.model.cfg.n_layers):
            hooks[f"L{layer}"] = (
                ...
                partial(freeze_hook, clean_attn_activation = cache[f"blocks.{layer}.attn.hook_pattern"])
            )
        
        return hooks
        
    def _get_current_hooks(self, layer:int, component: str, freezed_attn: Dict[str, Tuple[torch.Tensor, Any]], head: Optional[int] = None, position: Optional[int] = None):
        """
    ...
    def _process_model_run(self, layer, position, head, component, batch, freezed_attn, storage ):
        """
        run the model with the given hooks and store the logit in the storage
        """
        hooks = self._get_current_hooks(layer, component, freezed_attn, head=head, position=position)
    ...
    def ablate_single_len(self, length:int, component: str, normalize_logit: Literal["none", "softmax", "log_softmax"] = "none"):
        """
        ...
        if component in self.position_component:
            ...
            storage = HeadLogitStorage(n_layers=self.model.cfg.n_layers, length=1, n_heads=self.model.cfg.n_heads)
        else:
            ...
            for layer in range(self.model.cfg.n_layers):
                if component in self.position_component:
                    for position in range(length):
                        self._process_model_run(layer, position, None, component, batch, freezed_attn, storage)
                if component in self.head_component:
                    for head in range(self.model.cfg.n_heads):
                        self._process_model_run(layer, None, head, component, batch, freezed_attn, storage)
                
                freezed_attn.pop(f"L{layer}") # to speed up the process (no need to freeze the previous layer)
    ...
    def ablate(self, component: str, normalize_logit:Literal["none", "softmax", "log_softmax"] = "none"):
        """
        ...
        aggregated_result = [
            torch.cat([result[l][idx_tuple] for l in lengths], dim=-1) for idx_tuple in range(tuple_shape)
        ]
            ...
            for layer in range(self.model.cfg.n_layers):
                for position in range(result[0][layer].shape[0]):
                    data.append({
                        ...
                        "diff_std": (result[0][layer][position] - result[1][layer][position]).std().item(),
            ...
            for layer in range(self.model.cfg.n_layers):
                for head in range(self.model.cfg.n_heads):
                    data.append({
                        ...
                        "diff": (result[0][layer,0,head,:] - result[1][layer,0,head,:]).mean().item(),
                        "mem_std": result[0][layer,0,head,:].std().item(),
                        "cp_std": result[1][layer,0,head,:].std().item(),
                        "diff_std": (result[0][layer,0,head,:] - result[1][layer,0,head,:]).std().item(),
    ...
    def run_all(self, normalize_logit: Literal["none", "softmax", "log_softmax"] = "none"):
        """
