...
create_heatmap <- function(data, x, y, fill, high_color) {
  p <- create_heatmap_base(data, x, y, fill) +
    scale_fill_gradient2(low = "white", mid = "white", high = high_color, limits=c(0,17), name = "Logit") +
...
p_logit
ggsave("logit_last_new_ln.pdf", p_logit, width = 50, height = 30, units = "cm")
```
...
```{r}
ggsave("tmp/last_logit.pdf", p_logit, width = 50, height = 30, units = "cm")
```
...
for (i in c(1:length(head_pattern))) {
  ...
  if (c(layer,head) %in% factual_heads){
    color <-"#E31B23"
  }else{
    color <-  "#005CAB"
  }
  plot <- plot + plot_pattern(layer,head, data)
}
